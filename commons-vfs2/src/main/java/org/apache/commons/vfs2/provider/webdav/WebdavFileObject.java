...
public class WebdavFileObject<FS extends WebdavFileSystem> extends AbstractFileObject<FS> {
     ...
     * @throws IOException
     */
    private DavResource getResource() throws IOException {
        URI uri = getFullURI();
    ...
    /**
     * Check if the two URIs have same path/query, indicating identical files We
     * have to account for a potential discrepancy with a trailing slash
     * 
     * @param a
     *            first URI
     * @param b
                ...
                for (DavResource res : resources) {
                    if (!pathsEqual(stripHost(res.getHref()), getFullURI())) {
                        ...
                        if (resourceName != null && resourceName.length() > 0) {
                            FileName fname = fileSystem.getFileSystemManager().resolveName(getName(),
                                    UriParser.encode(resourceName), NameScope.CHILD);
    ...
    protected String getFullUrl() throws FileSystemException {
        URI uri = getFullURI();
        return uri.toString();
    }

    protected String getFullUrl(URLFileName file, boolean addIdentity) throws FileSystemException {
        URI uri = getFullURI(file, addIdentity);
        return uri.toString();
    }

    protected URI getFullURI() throws FileSystemException {
        return getFullURI((URLFileName) getName(), true);
    }

    protected URI getFullURI(URLFileName file, boolean addIdentity) throws FileSystemException {
        URI uri = null;
    ...
    protected String getHostRelativeUrl(URLFileName file) throws FileSystemException {
        URI uri = getHostRelativeURI(file);
        return uri.toString();
    }

    protected String getHostRelativeUrl() throws FileSystemException {
        URI uri = getHostRelativeURI();
        return uri.toString();
    }

    protected URI getHostRelativeURI() throws FileSystemException {
        URLFileName file = ((URLFileName) getName());
        return getHostRelativeURI(file);
    }

    protected URI getHostRelativeURI(URLFileName file) throws FileSystemException {
        URI uri = null;
